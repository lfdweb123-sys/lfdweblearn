    ...
    try {
      ...
      if (type === 'video') {
        ...
        updateLesson(moduleId, lessonId, {
          ...
          mediaUrl: `https://iframe.mediadelivery.net/embed/${process.env.NEXT_PUBLIC_BUNNY_STREAM_LIBRARY_ID}/${videoId}`,
        })
      } else {
        ...
        updateLesson(moduleId, lessonId, { mediaUrl })
      }

      toast.success('Fichier uploadé !', { id: toastId })

      // Sauvegarder après un court délai
      setTimeout(() => saveModules(modules), 500)
    } catch {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              {modules.reduce((s, m) => s + m.lessons.length, 0)} leçons au total
            </p>
          </div>
        </div>
        ...
        {modules.length === 0 && (
          <div className="bg-white rounded-2xl border border-slate-100 border-dashed p-12 text-center">
            ...
            <p className="text-slate-300 text-sm">
              Ajoutez votre premier module ci-dessous
            </p>
          </div>
        )}
              ...
              <div className="border-t border-slate-100">
                {module.lessons.map((lesson) => (
                  <LessonRow
                    ...
                    moduleId={module.id}
                    onUpdate={(changes) =>
                      updateLesson(module.id, lesson.id, changes)
                    }
                    onDelete={() => deleteLesson(module.id, lesson.id)}
                    onUpload={(file, type) =>
                      uploadLessonMedia(module.id, lesson.id, file, type)
                    }
        ...
        <div className="px-4 pb-4 space-y-3 bg-slate-50">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => {
                onUpdate({ isPreview: e.target.checked })
                onSave()
              }}
              className="rounded"
            />
